										...
										if(!in_array($page_id, array_merge(array($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["membership_options_page"], $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["login_welcome_page"], $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["file_download_limit_exceeded_page"]), preg_split("/[\r\n\t\s;,]+/", $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["specific_ids"]))))
											{
												...
												echo '<p style="margin-left:2px;"><strong>Who has access to this content?</strong></p>'."\n";
												...
												echo  /* By default, we allow public access to any Post/Page. */'<option value="Public">Public</option>'."\n";
												...
												for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["levels"]; $n++)
													...
													? '<option value="'.$n.'"'.((in_array($page_id, $pages[$n])) ? ' selected="selected"' : '').'>'.(($n === $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["levels"]) ? $GLOBALS['WS_PLUGIN__']['s2member']['o']['level'.$n.'_label'] : $GLOBALS['WS_PLUGIN__']['s2member']['o']['level'.$n.'_label']).'</option>'."\n"
										...
										if(!in_array($post_id, preg_split("/[\r\n\t\s;,]+/", $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["specific_ids"])))
											{
												...
												for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["levels"]; $n++)
													...
													? '<option value="'.$n.'"'.((in_array($post_id, $posts[$n])) ? ' selected="selected"' : '').'>'.(($n === $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["levels"]) ? $GLOBALS['WS_PLUGIN__']['s2member']['o']['level'.$n.'_label'] : $GLOBALS['WS_PLUGIN__']['s2member']['o']['level'.$n.'_label']).'</option>'."\n"
													: '<option value="" disabled="disabled">Level #'.$n.' (already protects "all" Posts of this type)</option>'."\n")
