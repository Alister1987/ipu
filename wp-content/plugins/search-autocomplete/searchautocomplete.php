...
class SearchAutocomplete {
	...
	//This method is responsible for actually querying the DATABASE.
	//Called each time the user enters a character to the input field.
	public function acCallbackNew($toDie = true) {
		global $wpdb,
		       $query,
				$wp_query;
		$resultsPosts = array();
        ...
        // getting all categories from sop and guidelines page
        $taxonomyCategories = get_the_terms(89, 'ipu_resource_category');
        $catMatches = array();


        foreach($taxonomyCategories as $taxCat) {
            if(strpos(strtolower($taxCat->name), strtolower($term)) !== false) {
                array_push($catMatches, $taxCat);
            }
        }
        ...
        foreach($catMatches as $catMatch) {
            $tObj = array(
                'title' => $catMatch->name,
                'url'   => '/professional/sop-and-guidelines/'.(strtolower(str_replace(' ', '_', $catMatch->name))),
		...
		if ( count( $this->options['autocomplete_posttypes'] ) > 0 ) {
			if ( ( function_exists( 'relevanssi_do_query' ) && ( $this->options['autocomplete_relevanssi'] !== 'false' ) ) ) {
				...
				//creates an array of posts based on a set of paramters.
                $args = array(
                ...
                //if($toDie){
				//	$args['numberposts'] = $this->options['autocomplete_numrows'];
                //} else {
                    $args['numberposts'] = -1;
                //}
           ...
           // $tags = implode(',', $termArr);

			//zn-5075
			$termArr = explode(' ', $term);
            $tags = $termArr;
            $tags = implode('-', $termArr);
            if(count($termArr) >= 1 && $termArr[0] != '') {
                $resultIds = array(1);
                for($i = 0; $i < count($tempPosts); $i++) {
                    $p = $tempPosts[$i];
                    if(isset($p->ID)) {
                ...
                if(count($resultIds) > 0) {
                    $args['post__not_in'] = $resultIds;
                }

                $tempPosts = array_merge( $tempPosts, get_posts($args) );

            }


            $downloadableList = array(
                'file',
                'postersandpromo',
                'circular',
                'letter',
			...
			foreach ( $tempPosts as $post ) {
                ...
                if(in_array($post->post_type, $downloadableList)) {
                    $downloadFile = wp_get_attachment_url(get_post_meta($post->ID, 'files', true));
                    if($downloadFile == '') {
                        $downloadFile = wp_get_attachment_url($post->file);
                    }
                }

				//Get the needed items - post assigned sections
				$ipu_categories = get_post_meta($post->ID, 'ipu_section', true);
				$cats = explode(',', $ipu_categories);

				//If post is assigned, published and permitted at current user level
				if(
					(($cats[0] == "" && $post->post_type == "page") || ($cats[0] !== "" && $post->post_type != "page")) &&
					'publish' === get_post_status($post->ID) &&
					(($post->post_type != "page" && is_post_permitted_by_s2member($post->ID)) || ($post->post_type == "page" && is_page_permitted_by_s2member($post->ID) ))
				) {
						$resultsPosts[] = $tObj;
				}
			}
		...
		if ( $this->options['autocomplete_sortorder'] == 'posts' ) {
			...
			//array_merge( $resultsPosts, $resultsTerms );
		} else {
			$results = $resultsPosts;
			//$results = array_merge( $resultsTerms, $resultsPosts );
		}

		//Calls function added to a filter hook. (name of the hook, value to be modified)
		// $results = $this->invalidPositionFilter($results);

        	//Count the total number of results
		$totalResults = count($results); //Total Results
		$totals[] = array(
			'total_results' => $totalResults
		);
		//return data in JSON Format
        	$res = array( 'total_results' => $totals );

	        if($toDie) {
	            $res['results'] = array_slice( $results, 0, $this->options['autocomplete_numrows'] );
	        } else {
	            $res['results'] = $results;
	        }
