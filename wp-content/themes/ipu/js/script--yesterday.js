...
//  masonry: { columnWidth: $container.width() / 9 }
//});
//
...
$('#business_form > .form_section .business_field_questionnaire ').each(function(index){
	$('.wpcf7-list-item').addClass('btn bg_btn');
	 ...
	 var pathForThisId = $(thisId).find('.bq_answer .bq_btn_sort_wrapper .btn');
		...
		$(pathForThisId).removeClass("bq_btn_yes", $checkbox.attr('checked'));
		...
		if (e.target.type == "checkbox") {
			...
			e.stopPropagation();
			$(this).filter(':has(:checkbox)').addClass('bq_btn_yes', $checkbox.attr('checked'));
		} else {
