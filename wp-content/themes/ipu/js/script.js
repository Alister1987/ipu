...
$(document).ready(function () {
    ...
    $('.bq_question .bq_answer .last').click(function(){
      //console.log('info drop down open');
       $(this).closest('.bq_inner_wrapper').children('.bq_info').slideToggle('slow');
    });
  ...
  $container.isotope({
   ...
   getSortData: {
      name: function( itemElem ) {
        return $( itemElem ).find('.gi_title' ).text().trim().toLowerCase();
      },
      role: '.gi_job',
      ...
      date: function( itemElem ) {
        var stamp = $(itemElem ).attr('data-time');
        if( !isNaN(parseInt(stamp) ) ) {
          return parseInt(stamp);
        } else if( !isNaN(new Date(stamp).getTime()) ) {
